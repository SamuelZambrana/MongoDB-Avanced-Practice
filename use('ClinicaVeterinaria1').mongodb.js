...
db.animales.insertMany([
  ...
  */
  ...
  db.animales.aggregate(
    [
      ...
      {
        $count: "cantidadPerrosConMoquillo"
      }
    ]
  )
  ...
  db.animales.aggregate(
    [
      ...
      },
      {
        $project: {
          _id: 0,
          mediaPrecio: { $concat: [{ $toString: "$mediaPrecio" }, " €"] }
        }
      }
    ]
  )
  ...
  */
  ...
  db.animales.aggregate(
    [
      ...
      },
      {
        $group: {
          _id: null,
          mediaVacunas: { $avg: { $size: "$historico_vacunas" } }
        }
      },
      {
        $project: {
          _id: 0,
          mediaVacunas: { $round: ["$mediaVacunas", 2] }
        }
      }
    ]
  )
    ...
    [
      {
        $project: {
          _id: 0,
          nombre: 1,
          cantidadVacunas: { $size: "$historico_vacunas" }
        }
      },
      {
        $sort: { cantidadVacunas: -1 }
      },
      {
        $limit: 1
      }
  ...
  db.animales.find(
    {
      notasAdicionales: { $regex: "dieta", $options: "i" }
    },
    {
      nombre: 1,
      raza: 1,
  ...
  //21. Obtener los colores y la cantidad de animales que hay de cada uno, donde la cantidad sea superior a 1.
  ...
  //23. Eliminar el atributo precio del animal "Paco".
